...
- **Changed:** `ai_task.generate_data` with a `structure` now always returns data of that structure, or fails with a clear error (*not valid JSON* or *did not match the requested structure*). Before, invalid JSON came back as a plain string and a mismatch was only a warning in the log. **Automations that relied on that lenient behaviour may now see an error.** MA-09.
